...
namespace TennisAcademyApp.Tests.Services
{
    ...
    public class BagCartServiceTests
    {
        ...
        public void SetUp()
        {
            ...
            dbContext = new TennisAcademyDbContext(options, httpContextAccessorMock.Object);

            // ==========================================
            // СЛОЖИ ТОЗИ БЛОК ТУК:
            // Автоматично попълване на Shadow Property-то в InMemory среда за тестовете
            dbContext.ChangeTracker.Tracked += (sender, e) =>
            {
                string auditColumnName = $"LastModified_{TennisAcademyDbContext.FacultyNumber}";
                if (e.Entry.Metadata.FindProperty(auditColumnName) != null &&
                    (e.Entry.State == EntityState.Added || e.Entry.State == EntityState.Modified))
                {
                    e.Entry.Property(auditColumnName).CurrentValue = DateTime.UtcNow;
                }
            };
            ...
            bagCartService = new BagCartService(dbContext, userManagerMock.Object, rankingServiceMock.Object);
        }
